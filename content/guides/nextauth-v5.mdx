...
Authentication is a crucial part of any web application. It is the process of verifying the identity of a user and protecting data.
...
You'll have a fully working authentication system in your Next.js application by following the steps below. It contains login, register, and logout functionality. And page like login
...
const publicRoutes = ["/", "/login", "/register"]; // Add your public routes here
...
### Layout for login and register UI.
...
This page is to /register route which will help users to create a new account and if the user is already authenticated, then it redirects the user to the main app ('/app' in this case)

```tsx:app/(auth)/register/page.tsx
import Logo from "@/components/logo";
import { Shapes } from "@/components/shapes";
import RegisterFormfrom "@/components/login-form";
import { auth } from "@/lib/auth";
import { redirect } from "next/navigation";
import React from "react";
...
This is the core logic of how register form is handled, it has typesafety, form validations, error handling (giving feedback for yours), the backend is the server actions handling custom logic

```tsx:register-form.tsx
"use client";
...
const signInAction = async (data: z.infer<typeof loginFormSchema>) => {
  try {
